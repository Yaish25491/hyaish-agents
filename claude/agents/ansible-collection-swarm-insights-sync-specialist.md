...
**Execute**:
```bash
# Navigate to hyaish-agents repository root
REPO_ROOT="$HOME/Documents/Git/hyaish-agents"
cd "$REPO_ROOT"
...
rm -rf ~/.claude/plugins/cache/local/hyaish-agents
rm -f ~/.claude/agents/hyaish-agents-plugin
...
**Plugin reinstalled**: `~/.claude/plugins/cache/local/hyaish-agents/1.0.0/`
...
Agent({
  ...
  subagent_type: "hyaish-agents/ansible-collection-swarm:insights-sync-specialist"
})
```
