...
Your `hyaish-agents` plugin is now ready for your team!
...
|---------|-------------|---------------|
| **Installation** | `/plugin install superpowers@claude-plugins-official` | `git clone` (or `/plugin install hyaish-agents@hyaish`) |
...
| **Namespace** | `superpowers:code-reviewer` | `hyaish-agents/SWARM-NAME:AGENT-NAME` |
...
- ✅ Namespaced properly (`hyaish-agents/SWARM-NAME:AGENT`)
...
```bash
# Create repo on GitHub: hyaish-agents
...
Documentation:
  ...
  cat ~/.claude/agents/hyaish-agents/ansible-collection-swarm/QUICKSTART.md

Repository:
  https://github.com/YOUR-USERNAME/hyaish-agents

Support:
  https://github.com/YOUR-USERNAME/hyaish-agents/issues
```
...
✅ Unified plugin structure (`hyaish-agents`)  
✅ Two complete swarms (universal + legacy)  
✅ Proper namespacing (`hyaish-agents/SWARM:AGENT`)  
✅ Slash command enabled (`/ansible-collection-swarm`)  
...
Your unified `hyaish-agents` plugin is:
1. ✅ Fully functional
...
**Current Location**: `~/.claude/agents/hyaish-agents/`  
